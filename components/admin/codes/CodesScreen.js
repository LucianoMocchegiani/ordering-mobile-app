...
function CardCode ({id, discount, code, navigation}){
    return(
        <TouchableOpacity
            onPress={()=>navigation.navigate('discountCodeDetail',{code:code})}>
...
function Header(){
    return(
        <View style={styles.containerHeader}>
            ...
            <Text style={{textTransform:"uppercase", marginLeft:5}}>Codigos de descuento</Text>
        </View>
    )
}

export default function RenderCodes({navigation}){
    const {discountCodesData}=useStorage()
    return (
        <View style={styles.containerAll}>
            <Header/>
            <View style={{width:width*0.95, flexDirection:'row', justifyContent: "space-between", marginBottom:10,}}>
                <ButtonHeader functionOnPress={()=>navigation.navigate('newDiscountCode')} buttonName={'Nuevo codigo'}/>       
            </View>
            <View>
                <FlatList
