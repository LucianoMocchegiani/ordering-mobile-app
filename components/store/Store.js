...
export default function Tienda({navigation}){
    ...
    useEffect(()=>{
        if(categorySelect.name != 'Ninguna'){
            getProducts(setLoading,setProductsData,'category_id',categorySelect.id)
        }else{
            getProducts(setLoading,setProductsData)
        }
    },[categorySelect])
    return (
        <View style={styles.container}>
            ...
            <View>
                ...
                <RenderProducts 
                    data={productsData} 
                    onScrollFunction={
                        categorySelect.name != 'Ninguna'?()=>getProductsOnScroll(setProductsData,productsData,'category_id',categorySelect.id)
                        :()=>getProductsOnScroll(setProductsData,productsData)}/>}
            </View>
        </View>
    )
}
